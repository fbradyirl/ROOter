...
function concatinfo(x)
	if x:sub(3, 6) == '0003' and x:sub(9, 12) ~= '0101' then
		m_concat = 'Msg# ' .. tonumber(x:sub(7, 8), 16) .. ',' .. tonumber(x:sub(11, 12), 16)
		m_concat = m_concat  .. '/' .. tonumber(x:sub(9, 10), 16)
	elseif x:sub(3, 6) == '0804' and x:sub(11, 14) ~= '0101' then
		m_concat = 'Msg# ' .. tonumber(x:sub(7, 10), 16) .. ',' .. tonumber(x:sub(13, 14), 16)
	...
	for octetcounter = 0, (octets - skip_octets - 1) do
		...
		if m_alphabet == 2 then
			if (2 + octetcounter) % 2 == 0 then
				...
				m_text = byte2utf8(bitor(bitleft(last_i, 8), i), m_text)
			end
		else
			m_text = byte2utf8(i, m_text)
		end
	end

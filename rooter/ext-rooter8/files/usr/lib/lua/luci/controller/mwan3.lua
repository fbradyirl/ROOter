...
function interfaceStatus()
	...
	if statusString ~= "" then
		...
		for wanName, interfaceState in string.gfind(statusString, "([^%[]+)%[([^%]]+)%]") do
			local wanInterfaceEnabled = ut.trim(sys.exec("uci -p /var/state get mwan3." .. wanName .. ".enabled"))
			local wanInterfaceName = ""
			if wanInterfaceEnabled == "1" then
				wanInterfaceName = ut.trim(sys.exec("uci -p /var/state get network." .. wanName .. ".ifname"))
			end
